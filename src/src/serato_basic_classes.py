...
class SeratoBaseClass:
    ...
    def load_input(self, input: Union[str, bytes]):
        ...
        Using mmap for ~reasons~
        doing the write/read dance for bytes
        (it was less trouble than figuring out how to standardize the downstream code.)
        TODO: fix this
        """
...
class SeratoObject(SeratoBaseClass):
    ...
    *** data_len:
    ...
    def get_data_by_tag(self, tag):
        try:
            return [
                d.object_data for d in self.object_data if d.object_type == tag
                ][0]
